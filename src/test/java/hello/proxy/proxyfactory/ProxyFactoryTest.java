...
public class ProxyFactoryTest {
    ...
    void interfaceProxy() {
        ...
        * proxyFactory.addAdvice(new TimeAdvice())
            ...
            * 이렇게 프록시가 제공하는 부가 기능 로직을 어드바이스라고( Advice )라 한다.
        ...
        */
        ...
        // AopUtils.isAopProxy(proxy) : 프록시 팩토리를 통해서 프록시가 생성되면 JDK 동적 프록시나, CGLIB 모두 true
        Assertions.assertThat(AopUtils.isAopProxy(proxy)).isTrue();

        //AopUtils.isJdkDynamicProxy(proxy) : 프록시 팩토리를 통해서 프록시가 생성되고, JDK 동적 프록시 인 경우 true
        Assertions.assertThat(AopUtils.isJdkDynamicProxy(proxy)).isTrue();

        //AopUtils.isCglibProxy(proxy) : 프록시 팩토리를 통해서 프록시가 생성되고, CGLIB 동적 프록시인 경우 true
    ...
    void proxyTargetTest() {
        ...
        // proxyTargetClass=true : 인터페이스 여부와 상관없이 구체 클래스(CGLIB) 기반 프록시로 강제한다.
